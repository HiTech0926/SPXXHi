...
// @author      HiTech0926 & Cinder & SPGoding & SPX Fellow
// @connect     *
// @connect     feedback.minecraft.com
// @connect     help.minecraft.net
// @connect     raw.githubusercontent.com
// @homepage    https://github.com/HiTech0926/SPXXHi
...
Converted at ${time.getFullYear()}-${padTime(time.getMonth() + 1) // why +1 javascript
  }-${padTime(time.getDate())} ${padTime(time.getHours())}:${padTime(time.getMinutes())} ${toHoursAndMinutes(time.getTimezoneOffset())}[/color][/size][/center]`;
...
/*Same contents,change if necessary.**/
  ...
  const translators = {
    ...
    code: (input, ctx) => {
      return quoteTreatment(input, [['[color=Silver][/color]', '`', /`/]]);
    }
  };
  ...
  const converters = {
    ...
    code: async (ele, ctx) => {
      const prefix = ctx.multiLineCode ? '[code]' : '[bgcolor=#f1edec][color=#7824c5]';
      const suffix = ctx.multiLineCode ? '[/code]' : '[/color][/bgcolor]';
    ...
    div: async (ele, ctx) => {
      ...
      if (ele.classList.contains('text-center')) {
        ...
        /*if (shouldUseAlbum(slides)) {
          ...
          ans = `[center]${slides.map(([url, caption]) => `[img]${url}[/img]\n${caption}`).join('\n')}[/center]\n`;
        } else {
          ans = '';
        }
    ...
    dl: async (ele, ctx) => {
      const ans = `\n\n${await converters.recurse(ele, ctx)}\n【本文排版借助了：[url=https://github.com/HiTech0926/SPXXHi][color=#388d40][u]SPXXHi[/u][/color][/url] 用户脚本 v${spxxhiVersion}】\n\n`;
      return ans;
    },
    dd: async (ele, ctx) => {
      let ans = '';
  ...
  function markdownToBbcode(value) {
    return value.replace(/`([^`]+)`/g, '[bgcolor=#f1edec][color=#7824c5]$1[/color][/bgcolor]');
  }
  /**
  ...
  async function minecraftNet() {
    ...
    if (url.match(/^https:\/\/www\.minecraft\.net\/(?:[a-z-]+)\/article\//)) {
      ...
      button.classList.add('mc-button__primary', 'mc-button__green-s1', 'spxxhi-userscript-ignored');
  ...
  function getHeroImage(html, articleType) {
    const category = articleType ? `\n[color=White][bgcolor=Black][b]${articleType}[/b][/bgcolor][/color][/center]` : '';
    ...
    const src = img.src;
    const ans = `[center][img=1200,513]${resolveUrl(src)}[/img]\n${category}\n`;
    return ans;
  }
  /**
  ...
  function getHelpContent(controlDOM) {
    ...
    button.onclick = async () => {
        ...
        bbcode += `[b]【${ctx.translator} 译自[url=${ctx.url}][color=#388d40][u] help.minecraft.net 上的 ${title}[/u][/color][/url]】[/b]\n【本文排版借助了：[url=https://github.com/HiTech0926/SPXXHi][color=#388d40][u]SPXXHi[/u][/color][/url] 用户脚本 v${version}】\n`;
...
【本文排版借助了：[url=https://github.com/HiTech0926/SPXXHi][color=#388d40][u]SPXXHi[/u][/color][/url] 用户脚本 v${spxxhiVersion}】\n\n${footer}`;
  ...
  [tr][td]
  [img=44,44]${ProfilePictures.get(tweet.userTag) || '<不支持的头像，请手动添加图片链接>'}[/img][size=15px][b][color=${foregroundColor}]${tweet.userName}[/color][/b]
